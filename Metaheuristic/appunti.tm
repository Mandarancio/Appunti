...
<\body>
  ...
  <section|Transizione di fase nei problemi di ottimizzazione>

  Vogliamo studiare il comportamento di una metaueristica in un problema a
  difficolta' regolabile.

  Vedremo che ci saranno dei cambiamenti di comportamento a soglia
  (transizioni di fase) in funzione del parametro che definisce la
  difficolta' del problema.\ 

  In questo caso studieremo dei problemi di <underline|soddisfazione>, dove
  e' possibile calcolare teoricamente la probabilita' di avere una soluzione.

  \ Vedremo che queste probabilta passa brutalmente da 1 a 0 e che la
  metaeuristica di esplorazione passa da un regime lineare a uno
  esponenziale.

  Questa transizione riflette il modo in cui le soluzioni popolano lo spazio
  di ricerca.

  <subsection|Problemi di soddisfazione (SAT)>

  Un problema di soddisfazione (<with|font-series|bold|SAT>) e' composto da
  <math|M> equazioni booleane (logiche) con <math|N> variabili
  <math|x<rsub|i>\<in\><around*|{|0,1|}>>.

  Esiste una combinazione delle <math|N> variabili che soddisfa le <math|M>
  equazioni?

  Per ogni schelta di <math|x<rsub|i>\<nocomma\>,i=1,\<ldots\>,N> possiamo
  definire una energia <math|E >uguale al numero di equazioni non
  soddisfatte. L'obbiettivo di un <with|font-series|bold|SAT> e' quindi
  quello di minimizzare <math|E> e trovare <math|E=0,> tutte le equazioni
  soddisfatte.

  <subsubsection|Il problema XORSAT>

  E' un caso particolare di <with|font-series|bold|SAT> dove solo l'operatore
  <math|xor> e' utilizzato.

  In pratica lo xor e' uguale a l'addizione a modulo due e per questo useremo
  il simbolo +\ 

  <\equation*>
    x<rsub|1> XOR x<rsub|2>=x<rsub|1>+x<rsub|2>\ 
  </equation*>

  Esempio:

  <\equation*>
    <around*|{|<tabular|<tformat|<table|<row|<cell|x<rsub|1>+x<rsub|2>+x<rsub|3>=1>>|<row|<cell|x<rsub|2>+x<rsub|4>=0>>|<row|<cell|x<rsub|1>+x<rsub|4>=1>>>>>|\<nobracket\>>
  </equation*>

  Questo e' un problema<with|font-series|bold| XORSAT> con <math|N=4> e
  <math|M=3>. E in particolare le soluzioni possibili sono:

  <\equation*>
    <around*|(|x<rsub|1>,x<rsub|2>,x<rsub|3>,x<rsub|4>|)>=<around*|{|<tabular|<tformat|<table|<row|<cell|<around*|(|1,0,0,0|)>>>|<row|<cell|<around*|(|0,1,0,1|)>>>>>>|\<nobracket\>>
  </equation*>

  E' anche possibile difinire problemi impossobili:

  <\equation*>
    <around*|{|<tabular|<tformat|<table|<row|<cell|x<rsub|1>+x<rsub|2>+x<rsub|3>=0>>|<row|<cell|x<rsub|1>+x<rsub|2>+x<rsub|3>=1>>>>>|\<nobracket\>>
  </equation*>

  I problemi di tipo <with|font-series|bold|XORSAT> si possono risolvere in
  tempo polimoniale utilizzando l'eliminazione di Gauss in aritmetica a
  modulo 2. La complessita algoritmica sara' <math|O<around*|(|N<rsup|3>|)>>
  (con <math|M=N>).

  Questo non e' evidentemente generale per i problemi
  <with|font-series|bold|SAT> (dato che sono non-lineari).

  Anche nel caso un problema <with|font-series|bold|XORSAT> non sia
  soddisfabile utilizzando il metodo di Gauss e' possibile saperlo e sapere
  quante equazioni sono soddisfabili.

  <\subsubsection>
    <math|k>-XORSAT
  </subsubsection>

  Sono una sottocategoria di problemi <with|font-series|bold|XORSAT> in cui
  ogni equazione a esattamente <math|k> parametri sui <math|N> possibile.

  Esempio:

  <\equation*>
    <around*|{|<tabular|<tformat|<table|<row|<cell|x<rsub|1>+x<rsub|4>=0>>|<row|<cell|x<rsub|2>+x<rsub|3>=0>>|<row|<cell|x<rsub|4>+x<rsub|5>=1>>>>>|\<nobracket\>>
  </equation*>

  Questo e' un problema <math|<with|font-series|bold|k-XORSAT>> con
  <math|k=2>, <math|N=5> e <math|M=3>.

  Possiamo analizzare matematicamente per <math|k=1,2, 3> la probabilita' che
  un sistema <with|font-series|bold|<math|k>-XORSAT> costruito a casa abbaia
  una soluzione, <math|P<around*|(|N,M,k|)>>.

  Si definisce un parametro <math|\<alpha\>>:

  <\equation*>
    \<alpha\>=<frac|M|N>
  </equation*>

  E per ogni scelta di <math|k> ci si interesera' alla probabilita'
  <math|P<around*|(|N,\<alpha\>|)>>. Il parametro <math|\<alpha\>> permettera
  di configurare la difficolta' del problma. Se <math|\<alpha\>> e' piccolo
  (<math|M\<ll\>N>) ci saranno dunque poche restrizioni e quindi il problema
  sara' facile. Al contrario piu' <math|M> sara' grande rispetto a <math|N>
  la difficolta' aumentera e ad un certo punto
  (<math|\<alpha\>\<gtr\>\<alpha\><rsub|critico>>) non ci saranno piu'
  soluzioni.

  Si trova che <math|k\<geqslant\>3> si ha il comportamento seguente:

  <with|gr-mode|<tuple|edit|text-at>|gr-frame|<tuple|scale|1cm|<tuple|0.150033gw|0.140033gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|cartesian|<point|0|0>|1>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-color|red|<graphics||<line|<point|0|6>|<point|6.0|6.0>|<point|6.0|0.0>>|<with|color|red|<line|<point|0|6>|<point|4.0|6.0>|<point|5.0|5.7>|<point|6.0|4.0>|<point|6.0|3.0>|<point|6.0|2.0>|<point|7.0|0.4>|<point|8.0|0.0>|<point|12.3|0.0>>>|<with|color|blue|<line|<point|0|6>|<point|2.01556968026813|6.0>|<point|10.9|0.0>|<point|12.6|0.0>>>|<with|color|blue|<text-at|N\TM|<point|7|7>>>|<with|color|red|<text-at|N
  piccolo|<point|7.5|5.5>>>>>

  Per <math|k=3> <math|\<alpha\><rsub|c>=0.9197>.

  <subsubsection|Modelli Statistici>

  Possiamo generare in maniera aleatoria dei problemi <math|k-XORSAT> per
  <math|k,N,\<alpha\>> fissati in due modi diversi:

  <\enumerate>
    <item>Si scelgono <math|k> variabili distinte a caso tra le <math|N>, e
    un secondo membro <math|\<nu\>> e si creano delle equazioni:

    <\equation*>
      x<rsub|i,1>+x<rsub|i,2>+\<ldots\>+x<rsub|i,k>=\<upsilon\><rsub|i>
    </equation*>

    Si ripete questa operazione <math|M>=<math|\<alpha\>N> volte e si avra'
    generato un problema <math|k-XORSAT> aletaorio.

    <item>Si costruiscono tutte le possibili equazioni con <math|k> variabili
    distinte tra le <math|N>, ce ne sono <math|H>:

    <\equation*>
      H=2<around*|(|<tabular|<tformat|<table|<row|<cell|N>>|<row|<cell|k>>>>>|)>
    </equation*>

    Ogni una di queste equazioni avra' probabilita' di essere scelta
    <math|p=<frac|M|H>>, in questo modo si avranno in <underline|media>
    <math|M> equazioni generate.
  </enumerate>

  Possiamo quindi generare <math|\<cal-N\>> problemi
  \ (<math|\<cal-N\>=10000>) per ognuno di essi applicheremo la eleminazione
  gaussiana per determinare se e' risolvibile e quindi e' possibile calcolare\ 

  <\equation*>
    P<around*|(|N,\<alpha\>|)>=<frac|n problemi risolvibili|\<cal-N\>>
  </equation*>

  <subsubsection|Comportamento di una metaueristica (RWSAT)>

  Studiamo in questo paragrafo il comportamento della metaeuristica chiamata
  <with|font-series|bold|RWSAT> (Random Walk SAT) applicata a problemi di
  tipo <math|3-XORSAT>.

  In particolare siamo interessati alla sua capacita' di trovare delle
  soluzioni in funzione di <math|\<alpha\>>.

  L'algoritmo RWSAT sceglie inizialmente di inizializzare a caso le <math|N>
  variabili. Inseguito si guardano le equazioni che non sono state
  soddisfatte da questa asseganzione. A questo punto i cambia una delle 3
  variabili di una di queste equazioni.\ 

  In questo modo e' garantito che questa equazione sia adesso soddisfatta.

  Avendo cambiato una variabile avremo forse cambiato il risultato di altre
  equazioni. Inoltre non c'e' garanzia che l'algoritmo trovi la soluzione.

  <paragraph|Pseudo Codice>

  <\render-code>
    <\with|font|courier|font-base-size|8>
      for x in N do
      ...
      while (E\<gtr\>0 and t\<less\>tmax) do

      \ \ e = random_unsudisfied_equation()
  ...
  Se <math|E=0> siamo sicuri del risultato, altrimenti non siamo sicuri se il
  sistema non abbia realmente la soluzione o la RWSAT non sia stato in grado
  di trovare la soluzione.

  Se si repete <math|T> volte RWSAT \ sullo stesso problema e per <math|T>
  RWSAT trovi <math|E\<gtr\>0> la probabilita' che il problema sia
  risolvibile e':

  <\equation*>
    p\<less\>e<rsup|-T<around*|(|<frac|3|4>|)><rsup|N>>
  </equation*>

  <\with|prog-scripts|gnuplot>
    <image|<tuple|<#>|ps>||||>

    <script-eval|set xlabel 't'; set ylabel 'E/M'; set title 'alpha grande';
    plot [0:2][0:1.5] exp(-x*2)+0.2 title 'N
    grande',exp(-x*2)+(1-exp(-x*2))*rand(0)/4+0.2 title 'N normale'>
  </with>

  \;

  Se <math|\<alpha\>\<less\>0.33>\ 

  <\equation*>
    \<less\>T<rsub|sol>\<gtr\>=N*t<rsub|res>
  </equation*>

  Il tempo di resuluzione ha andamento lineare quindi addirittura meglio del
  algoritmo di Gauss (<math|O<around*|(|N<rsup|3>|)>>) mentre superato il
  valore di soglia l'algoritmo avra' andamento esponenziale.\ 

  In questo caso si parlera' dunque di transizione di fase.

  <subsection|Algoritmo di Back-Traking>

  In questo caso si utilizza un algoritmo di Back-Tracking per risolvere un
  problema di tipo <with|font-series|bold|SAT>.

  Si assegnano successivamente le <math|N> variabili secondo un albero che
  viene esplorato in profondita'.\ 

  Se una assegnizaione parziale e' in contraddizione con una gia' fatta si
  riassegna utilizzando le regole del BT.

  Questo e' un metodo esatto di risoluzione.\ 

  Si stima le performance del BT come il numero di nodi generati per trovare
  la soluzione.

  <\equation*>
    \<less\>T<rsub|sol>\<gtr\>=<around*|{|<tabular|<tformat|<table|<row|<cell|N*t<rsub|res>>|<cell|\<alpha\>\<less\><frac|2|3>>>|<row|<cell|e<rsup|<around*|(|N*t<rsub|res>|)>>>|<cell|\<alpha\>\<geqslant\><frac|2|3>>>>>>|\<nobracket\>>
  </equation*>

  Ma una volta raggiunto raggiunto <math|\<alpha\>\<geqslant\>\<alpha\><rsub|c>>
  a questo punto la velocita' aumenta ancora (in quanto ci sono sempre piu'
  contraddizioni e quindi nodi che si possono scartare).

  Possiamo comprendere il comportamento nelle due regione come il fatto che
  con <math|\<alpha\>\<less\><frac|2|3>> la soluzione viene trovata
  esplorando una sola parte dell'albero.\ 

  Invece per trovare una soluzione con <math|\<alpha\>\<geqslant\><frac|2|3>>
  l'algoritmo deve esplorare tutto o quasi l'albero per trovare la soluzione.\ 
...
<\references>
  <\collection>
    ...
    <associate|auto-63|<tuple|8|?>>
    <associate|auto-64|<tuple|8.1|?>>
    <associate|auto-65|<tuple|8.1.1|?>>
    <associate|auto-66|<tuple|8.1.2|?>>
    <associate|auto-67|<tuple|8.1.3|?>>
    <associate|auto-68|<tuple|8.1.4|?>>
    <associate|auto-69|<tuple|8.1.4.1|?>>
    <associate|auto-7|<tuple|2.1.1|6>>
    <associate|auto-70|<tuple|8.2|?>>
    <associate|auto-71|<tuple|8.3|?>>
...
<\auxiliary>
  <\collection>
    <\associate|figure>
      ...
      <tuple|normal|Errore medio/sforzo computazionale|<pageref|auto-60>>

      <tuple|normal|Probabilita' di successo / sforzo|<pageref|auto-61>>
    </associate>
    <\associate|table>
    ...
    <\associate|toc>
      ...
      <with|par-left|<quote|1tab>|7.2<space|2spc>Il teorema ``No Free Lunch''
      (NFL) <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-62>>

      <vspace*|1fn><with|font-series|<quote|bold>|math-font-series|<quote|bold>|8<space|2spc>Transizione
      di fase nei problemi di ottimizzazione>
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-63><vspace|0.5fn>

      <with|par-left|<quote|1tab>|8.1<space|2spc>Problemi di soddisfazione
      (SAT) <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-64>>

      <with|par-left|<quote|2tab>|8.1.1<space|2spc>Il problema XORSAT
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-65>>

      <with|par-left|<quote|2tab>|8.1.2<space|2spc><with|mode|<quote|math>|k>-XORSAT
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-66>>

      <with|par-left|<quote|2tab>|8.1.3<space|2spc>Modelli Statistici
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-67>>

      <with|par-left|<quote|2tab>|8.1.4<space|2spc>Comportamento di una
      metaueristica (RWSAT) <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-68>>

      <with|par-left|<quote|4tab>|Pseudo Codice
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
